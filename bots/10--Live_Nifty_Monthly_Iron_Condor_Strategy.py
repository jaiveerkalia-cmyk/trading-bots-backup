...
# Storage Directories & Persistent State Paths (Mounted to /app/data/)
gd_path = '/app/data/'
auth_path = '/app/config/auth.txt'
results_dir = os.path.join(gd_path, 'Nifty_Monthly_Iron_Condor_Strategy_Results')
...
def run_trading_worker():
    ...
    if today_date.weekday() in [5, 6]:
        print("Weekend detected. Terminating worker.")
        return
    ...
    # Resolve Active Execution Mode from Frozen State if Trade is Open
    active_trade_live_mode = state.get('live_mode', live_mode)
    mode_tag = "LIVE TRADING (Real Capital Deployed)" if active_trade_live_mode == 1 else "PAPER TRADING (Orderbook Depth Fills)"
    ...
    # Synchronize to exact upcoming minute boundary (:00 seconds)
    def sleep_to_minute_boundary():
        t_now = datetime.now(KOLKATA_TZ)
        sleep_dur = 60.0 - t_now.second - (t_now.microsecond / 1_000_000.0)
        if sleep_dur <= 0.05:
            sleep_dur += 60.0
        time.sleep(sleep_dur)

    sleep_to_minute_boundary()
    ...
    while True:
        ...
        # Periodic Spot Price Fetch with Stale Guard
        try:
            q_scrip = kite.ltp(f"NSE:{scrip}")
            ...
            if within_scan_window:
                ...
                if cand_exp:
                    ...
                    if setup:
                        ...
                        current_entry_mode = live_mode  # Freeze live_mode for this trade

                        state = {'status': 'ENTRY_PENDING', 'trade_id': trade_id, 'traded_expiries': state['traded_expiries'], 'live_mode': current_entry_mode}
                        save_trade_state(state)

                        # Phased Entry Execution: Long Wings First (Hedge Protection)
                        fill_lc = place_order(kite, setup['lc']['sym'], total_qty, 'BUY', current_entry_mode)
                        fill_lp = place_order(kite, setup['lp']['sym'], total_qty, 'BUY', current_entry_mode)
                        ...
                        if fill_lc['status'] != 'COMPLETE' or fill_lp['status'] != 'COMPLETE':
                            ...
                            if fill_lc['status'] == 'COMPLETE':
                                place_order(kite, setup['lc']['sym'], total_qty, 'SELL', current_entry_mode)
                            if fill_lp['status'] == 'COMPLETE':
                                place_order(kite, setup['lp']['sym'], total_qty, 'SELL', current_entry_mode)
                        ...
                        fill_sc = place_order(kite, setup['sc']['sym'], total_qty, 'SELL', current_entry_mode)
                        fill_sp = place_order(kite, setup['sp']['sym'], total_qty, 'SELL', current_entry_mode)
                        ...
                        if fill_sc['status'] != 'COMPLETE' or fill_sp['status'] != 'COMPLETE':
                            ...
                            if fill_sc['status'] == 'COMPLETE':
                                place_order(kite, setup['sc']['sym'], total_qty, 'BUY', current_entry_mode)
                            if fill_sp['status'] == 'COMPLETE':
                                place_order(kite, setup['sp']['sym'], total_qty, 'BUY', current_entry_mode)
                            place_order(kite, setup['lc']['sym'], total_qty, 'SELL', current_entry_mode)
                            place_order(kite, setup['lp']['sym'], total_qty, 'SELL', current_entry_mode)
                            state = {'status': 'IDLE', 'trade_id': None, 'traded_expiries': state['traded_expiries']}
                            save_trade_state(state)
                        ...
                        # Target & SL Calculations on ACTUAL Fill Prices
                        actual_net_credit_pts = (fill_sc['fill_price'] + fill_sp['fill_price']) - (fill_lc['fill_price'] + fill_lp['fill_price'])
                        actual_credit_inr = actual_net_credit_pts * total_qty
                        actual_target_pnl = actual_credit_inr * (call_profit_percent / 100.0)
                        actual_stop_pnl = -(actual_credit_inr * (call_loss_percent / 100.0))

                        call_wing_w = setup['lc']['strike'] - setup['sc']['strike']
                        put_wing_w = setup['sp']['strike'] - setup['lp']['strike']
                        max_w = max(call_wing_w, put_wing_w)
                        max_loss_pts = max_w - actual_net_credit_pts
                        max_loss_inr = max_loss_pts * total_qty
                        actual_rr_ratio = max_loss_pts / actual_net_credit_pts if actual_net_credit_pts > 0 else 999.0

                        # Print Trade Manifest Including Max Loss
                        print("\n" + "=" * 88, flush=True)
                        print(f"  [TRADE OPENED] {trade_id} | Mode: {'LIVE TRADING' if current_entry_mode == 1 else 'PAPER TRADING'}", flush=True)
                        print("=" * 88, flush=True)
                        print(f"  Expiry Date         : {cand_exp} ({setup['dte']} DTE) | Underlying Spot: {spot_price:.2f}", flush=True)
                        print(f"  Short Call Leg      : Strike {setup['sc']['strike']:.0f} CE | Fill: Rs. {fill_sc['fill_price']:.2f} ({setup['sc']['delta']:.2f}D)", flush=True)
                        print(f"  Long Call Wing      : Strike {setup['lc']['strike']:.0f} CE | Fill: Rs. {fill_lc['fill_price']:.2f} ({setup['lc']['delta']:.2f}D) [Width: {call_wing_w:.0f}]", flush=True)
                        print(f"  Short Put Leg       : Strike {setup['sp']['strike']:.0f} PE | Fill: Rs. {fill_sp['fill_price']:.2f} ({setup['sp']['delta']:.2f}D)", flush=True)
                        print(f"  Long Put Wing       : Strike {setup['lp']['strike']:.0f} PE | Fill: Rs. {fill_lp['fill_price']:.2f} ({setup['lp']['delta']:.2f}D) [Width: {put_wing_w:.0f}]", flush=True)
                        print(f"  Position Sizing     : {ratio_multiplier} lots ({total_qty} units) | Deployed Capital: Rs. {total_cap:,.2f}", flush=True)
                        print(f"  Actual Net Credit   : {actual_net_credit_pts:.2f} pts (Total Cash: Rs. {actual_credit_inr:,.2f})", flush=True)
                        print(f"  MAXIMUM LOSS        : {max_loss_pts:.2f} pts (Total Max Risk: Rs. {max_loss_inr:,.2f})", flush=True)
                        print(f"  Risk-to-Reward Ratio: 1:{actual_rr_ratio:.2f}", flush=True)
                        print(f"  Take-Profit (+{call_profit_percent:.0f}%): Rs. {actual_target_pnl:+,.2f} | Stop-Loss (-{call_loss_percent:.0f}%): Rs. {actual_stop_pnl:+,.2f}", flush=True)
                        print(f"  Hard Sunset Exit    : {exit_dte} DTE @ {rollover_time} IST", flush=True)
                        print("=" * 88 + "\n", flush=True)
                        ...
                        state = {
                            'status': 'ACTIVE',
                            'trade_id': trade_id,
                            'live_mode': current_entry_mode,  # Frozen execution mode
                            'expiry': cand_exp,
                            'entry_time': ts_str,
                            'entry_spot': spot_price,
                            'initial_credit_pts': actual_net_credit_pts,
                            'legs': legs_conf,
                            'call_wing_width': call_wing_w,
                            'put_wing_width': put_wing_w,
                            ...
                            'frozen_config': {
                                'live_mode': current_entry_mode,
                                'quantity': total_qty,
                                'setup_cost': total_cap,
                                'target_pnl': actual_target_pnl,
                                'stop_pnl': actual_stop_pnl,
        ...
        if state.get('status') == 'ACTIVE':
            ...
            active_trade_live_mode = state.get('live_mode', cfg.get('live_mode', live_mode))
            ...
            if target_hit or stop_hit or dte_cutoff_hit or friction_guard_hit:
                ...
                # Step 1: Buy to Close Short Legs
                f_sc_x = place_order(kite, legs['sc']['sym'], q_val, 'BUY', active_trade_live_mode)
                f_sp_x = place_order(kite, legs['sp']['sym'], q_val, 'BUY', active_trade_live_mode)

                exit_ts = now.strftime('%Y-%m-%d %H:%M:%S')
                exit_comm = 0.0

                if f_sc_x['status'] == 'COMPLETE':
                    chg = calc_single_execution_charges(q_val, f_sc_x['fill_price'], 'BUY', exchange)
                    exit_comm += chg
                    append_to_tradebook({
                        'Trade_ID': state['trade_id'], 'Timestamp': exit_ts, 'Symbol': symbol,
                        'Expiry': state['expiry'], 'Strike': legs['sc']['strike'],
                        'Option_Type': 'CE', 'Order_Side': 'BUY', 'Quantity': q_val,
                        'Order_Type': 'LIMIT', 'Fill_Price': f_sc_x['fill_price'],
                        'Order_ID': f_sc_x['order_id'], 'Charges_STT_Brok': chg, 'Action_Tag': 'EXIT_' + reason.split()[0].upper()
                    })
                    # Safe to close Long Call Hedge only after Short Call is confirmed closed
                    f_lc_x = place_order(kite, legs['lc']['sym'], q_val, 'SELL', active_trade_live_mode)
                    if f_lc_x['status'] == 'COMPLETE':
                        chg_lc = calc_single_execution_charges(q_val, f_lc_x['fill_price'], 'SELL', exchange)
                        exit_comm += chg_lc
                        append_to_tradebook({
                            'Trade_ID': state['trade_id'], 'Timestamp': exit_ts, 'Symbol': symbol,
                            'Expiry': state['expiry'], 'Strike': legs['lc']['strike'],
                            'Option_Type': 'CE', 'Order_Side': 'SELL', 'Quantity': q_val,
                            'Order_Type': 'LIMIT', 'Fill_Price': f_lc_x['fill_price'],
                            'Order_ID': f_lc_x['order_id'], 'Charges_STT_Brok': chg_lc, 'Action_Tag': 'EXIT_' + reason.split()[0].upper()
                        })
                else:
                    print(f"CRITICAL: Failed to close Short Call {legs['sc']['sym']}! Protective Long Call {legs['lc']['sym']} RETAINED TO PREVENT NAKED SHORT EXPOSURE!", flush=True)

                if f_sp_x['status'] == 'COMPLETE':
                    chg = calc_single_execution_charges(q_val, f_sp_x['fill_price'], 'BUY', exchange)
                    exit_comm += chg
                    append_to_tradebook({
                        'Trade_ID': state['trade_id'], 'Timestamp': exit_ts, 'Symbol': symbol,
                        'Expiry': state['expiry'], 'Strike': legs['sp']['strike'],
                        'Option_Type': 'PE', 'Order_Side': 'BUY', 'Quantity': q_val,
                        'Order_Type': 'LIMIT', 'Fill_Price': f_sp_x['fill_price'],
                        'Order_ID': f_sp_x['order_id'], 'Charges_STT_Brok': chg, 'Action_Tag': 'EXIT_' + reason.split()[0].upper()
                    })
                    # Safe to close Long Put Hedge only after Short Put is confirmed closed
                    f_lp_x = place_order(kite, legs['lp']['sym'], q_val, 'SELL', active_trade_live_mode)
                    if f_lp_x['status'] == 'COMPLETE':
                        chg_lp = calc_single_execution_charges(q_val, f_lp_x['fill_price'], 'SELL', exchange)
                        exit_comm += chg_lp
                        append_to_tradebook({
                            'Trade_ID': state['trade_id'], 'Timestamp': exit_ts, 'Symbol': symbol,
                            'Expiry': state['expiry'], 'Strike': legs['lp']['strike'],
                            'Option_Type': 'PE', 'Order_Side': 'SELL', 'Quantity': q_val,
                            'Order_Type': 'LIMIT', 'Fill_Price': f_lp_x['fill_price'],
                            'Order_ID': f_lp_x['order_id'], 'Charges_STT_Brok': chg_lp, 'Action_Tag': 'EXIT_' + reason.split()[0].upper()
                        })
                else:
                    print(f"CRITICAL: Failed to close Short Put {legs['sp']['sym']}! Protective Long Put {legs['lp']['sym']} RETAINED TO PREVENT NAKED SHORT EXPOSURE!", flush=True)

                # Settle Trade only if all 4 legs exited cleanly
                if f_sc_x['status'] == 'COMPLETE' and f_sp_x['status'] == 'COMPLETE':
                    ...
                    print("TRADE EXIT INCOMPLETE: Short legs failed to clear. Keeping position under EXIT_PENDING lock.")
                    state['total_commission_paid'] += exit_comm
                    save_trade_state(state)
            ...
            if enable_adjustments and cur_dte > cfg['exit_dte'] and cooldown_ok and timing_ok:
                ...
                if state['down_rolls_done'] == 0 and spot_price <= legs['sp']['strike']:
                    ...
                    if valid_roll:
                        # 1. Buy back old Short Call
                        f_sc_c = place_order(kite, legs['sc']['sym'], q_val, 'BUY', active_trade_live_mode)
                        if f_sc_c['status'] == 'COMPLETE':
                            # 2. Sell out old Long Call
                            f_lc_c = place_order(kite, legs['lc']['sym'], q_val, 'SELL', active_trade_live_mode)
                            comm_close = (
                            ...
                            # 3. Buy New Long Call Wing First
                            f_new_lc = place_order(kite, row_lc['tradingsymbol'], q_val, 'BUY', active_trade_live_mode)
                            if f_new_lc['status'] == 'COMPLETE':
                                # 4. Sell New Short Call Leg
                                f_new_sc = place_order(kite, row_sc['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                                if f_new_sc['status'] == 'COMPLETE':
                                    comm_open = (
                                    ...
                                    state['total_commission_paid'] += (comm_close + comm_open)
                                    ...
                                    # New Short Call failed - Liquidate new Long Call to avoid holding naked long
                                    print("CRITICAL: New Short Call failed to fill! Liquidating new Long Call...", flush=True)
                                    place_order(kite, row_lc['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                            else:
                                print("CRITICAL: New Long Call hedge failed to fill! Roll aborted before selling new short call.", flush=True)
                        else:
                            print("CRITICAL: Failed to buy back old Short Call! Old spread maintained intact.", flush=True)

                # SCENARIO A2: Downside Stage 2 (Breaches Long Put Wing -> Iron Fly)
                            ...
                            if f_new_lc['status'] == 'COMPLETE':
                                ...
                                if f_new_sc['status'] == 'COMPLETE':
                                    ...
                                    if (cfg['target_pnl'] - final_net_terminal_pnl) <= (comm_terminal_exit * 1.5):
                                        ...
                                        f_sc_t = place_order(kite, row_sc['tradingsymbol'], q_val, 'BUY', active_trade_live_mode)
                                        f_sp_t = place_order(kite, legs['sp']['sym'], q_val, 'BUY', active_trade_live_mode)
                                        if f_sc_t['status'] == 'COMPLETE' and f_sp_t['status'] == 'COMPLETE':
                                            f_lc_t = place_order(kite, row_lc['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                                            f_lp_t = place_order(kite, legs['lp']['sym'], q_val, 'SELL', active_trade_live_mode)
                                    ...
                                    print("CRITICAL: New Short Call failed during Iron Fly conversion! Selling new Long Call...", flush=True)
                                    place_order(kite, row_lc['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                            else:
                                print("CRITICAL: New Long Call hedge failed during Iron Fly conversion!", flush=True)
                        else:
                            print("CRITICAL: Failed to buy back old Short Call for Iron Fly! Old spread kept intact.", flush=True)

                # SCENARIO B: Upside Breach (Breaches Short Call -> Iron Fly)
                    ...
                    if valid_roll:
                        f_sp_c = place_order(kite, legs['sp']['sym'], q_val, 'BUY', active_trade_live_mode)
                        if f_sp_c['status'] == 'COMPLETE':
                            f_lp_c = place_order(kite, legs['lp']['sym'], q_val, 'SELL', active_trade_live_mode)
                            comm_close = (
                            ...
                            f_new_lp = place_order(kite, row_lp['tradingsymbol'], q_val, 'BUY', active_trade_live_mode)
                            if f_new_lp['status'] == 'COMPLETE':
                                f_new_sp = place_order(kite, row_sp['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                                if f_new_sp['status'] == 'COMPLETE':
                                    comm_open = (
                                    ...
                                    print("CRITICAL: New Short Put failed during Upside conversion! Liquidating new Long Put...", flush=True)
                                    place_order(kite, row_lp['tradingsymbol'], q_val, 'SELL', active_trade_live_mode)
                            else:
                                print("CRITICAL: New Long Put hedge failed during Upside conversion!", flush=True)
                        else:
                            print("CRITICAL: Failed to buy back old Short Put! Old spread kept intact.", flush=True)
