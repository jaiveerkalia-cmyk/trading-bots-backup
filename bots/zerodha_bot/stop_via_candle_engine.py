...
This engine then:
  ...
  2. Computes a real Stop-Market trigger 1 tick beyond that candle's high or low, mirroring
     ...
     INDEX-SPECIFIC and user-configurable from the UI (config.get_enter_via_stop_tick(),
     reading params['enter_via_stop_tick_nifty']/['enter_via_stop_tick_sensex'] -- since
     NIFTY and SENSEX trade in different point scales); premium-level tick is a single flat
     constant (config.ENTER_VIA_STOP_PREMIUM_TICK) since both indices' options trade in the
     same rupee-premium scale regardless of underlying index.
...
from config import shared_state, params, INDICES, get_eval_token, get_eval_price, get_enter_via_stop_tick, ENTER_VIA_STOP_PREMIUM_TICK
...
class StopViaCandleEngine:
    ...
    def _try_fetch(self, job, now):
        ...
        # Tick buffer: index-specific and user-configurable from the UI for entry/index_stop
        # jobs (config.get_enter_via_stop_tick(), reading params['enter_via_stop_tick_nifty']/
        # ['enter_via_stop_tick_sensex']), since NIFTY and SENSEX trade in different point
        ...
        if job['kind'] == 'premium_stop':
            ...
            tick = get_enter_via_stop_tick(job['index_name'])
