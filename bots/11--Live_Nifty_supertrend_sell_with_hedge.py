...
def sell_fn(kite, zerodha_instruments_list, expiry):
    ...
    intraday_positions = pd.read_csv(gd_path + 'Nifty_sell_supertrend_results/Intraday_options_tradebook.csv')
    final_position_df = pd.read_csv(gd_path + 'Nifty_sell_supertrend_results/Final_daily_pnl.csv')
    ...
    #####GET TOKEN
    time.sleep(1207 - time.localtime().tm_sec )
    print(datetime.now())
    ...
    while True:
        try:
            ...
            if latest_fetched_time >= target_candle_time:
                print(f"Target {TARGET_HOUR}:{TARGET_MINUTE} candle verified! Latest fetched: {latest_fetched_time}")
                break
            else:
...
##    df_ohlc['atr'] =  talib.ATR(df_ohlc.high, df_ohlc.low, df_ohlc.close, timeperiod= 10)
    ...
    df_ohlc.drop(['prev_close', 'tr1', 'tr2', 'tr3', 'tr'], axis=1, inplace=True)##    df_ohlc = SuperTrend(df_ohlc, 10,4)
    df_ohlc = calculate_supertrend_numba(df_ohlc, 10,4)
...
##    if df_ohlc.iloc[-1].stx == 'down': 
##        sell_call_flag = 1
##        sell_put_flag = 0
##    if df_ohlc.iloc[-1].stx == 'up' : 
##        sell_put_flag = 1
##        sell_call_flag = 0
    ...
    for pos_num in range(0,4):
        ...
        if pos_num > 2 or (pos_num >1 and  final_profit > 0):
            ...
            final_position_df.to_csv(gd_path +'Nifty_sell_supertrend_results/Final_daily_pnl.csv', index=False)
            print('day_over')
            return()
        ...
        #####GET INITIAL ENTRY PUT PRICE
            ...
            intraday_positions.to_csv(gd_path+'Nifty_sell_supertrend_results/Intraday_options_tradebook.csv', index=False)
        ...
        while True:
            ...
            print(f"{datetime.now()}| Realized Profit: {round(final_profit, 2)}| PE: {current_entry_put_price} CE: {current_entry_call_price}| P&L: PE={sell_put_profit} CE={sell_call_profit}")
...
##                        try:
##                            a1 = kite.ltp('NFO:'+sym_put)
##                            exit_sell_put_price = (a1['NFO:'+sym_put]['last_price'])
##                            break
##                        except Exception as e:
##                            print(e)
...
##                        try:
##                            a1 = kite.ltp('NFO:'+sym_call)
##                            exit_sell_call_price = (a1['NFO:'+sym_call]['last_price'])
##                            break
##                        except Exception as e:
##                            print(e)
...
##                    final_profit = final_profit + qty*(exit_hedge_call_price-entry_hedge_call_price) - commission(qty, entry_hedge_call_price, exit_hedge_call_price)
            ...
            if (sell_put_flag == 1 and current_entry_put_price >= 1.5*entry_put_price) or (sell_put_flag == 1 and pos_num >=1 and sell_put_profit >= 1.1*abs(final_profit)):
...
##                final_profit = final_profit + qty*(exit_hedge_put_price-entry_hedge_put_price) - commission(qty, entry_hedge_put_price, exit_hedge_put_price)
            ...
            if (sell_call_flag == 1 and current_entry_call_price >= 1.5*entry_call_price) or (sell_call_flag == 1 and pos_num >=1 and sell_call_profit >= 1.1*abs(final_profit)):
