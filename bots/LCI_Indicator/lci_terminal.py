...
app.clientside_callback(
    """function(n_clicks) { if(n_clicks && n_clicks>0) { try { window.audioCtx = new (window.AudioContext || window.webkitAudioContext)(); if(window.audioCtx.state === 'suspended') { window.audioCtx.resume(); } var a = new Audio(); a.play().then(()=>{a.pause();}).catch(()=>{}); } catch(e) { } return [true, "🔊 Audio Active - Alerts Enabled"]; } return [false, "🔇 Click to Enable Audio"]; }""",
    [Output('audio-enabled','data'), Output('audio-enable-btn','children')], Input('audio-enable-btn','n_clicks')
)
app.clientside_callback(
    """function(trigger, sound_url, duration, audio_enabled) { if(!audio_enabled) { return window.dash_clientside.no_update; } if(trigger && sound_url && sound_url !== 'none') { var audio = new Audio(sound_url); audio.loop = true; audio.play().catch(function(e) { }); setTimeout(function(){ audio.pause(); audio.currentTime=0; }, (duration||6)*1000); } return window.dash_clientside.no_update; }""",
    Output('audio-dummy','children'), Input('sound-trigger','data'), State('sound-dropdown','value'), State('sound-duration','value'), State('audio-enabled','data')
)
