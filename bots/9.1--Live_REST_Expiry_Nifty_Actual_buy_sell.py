...
def commission(amount, buy_price, sell_price):
    turnover = (amount * buy_price) + (amount*sell_price)
    zerodha_brokerage = 40
    STT = 0.001 * amount*sell_price
    Exchange_txn_charge =  0.0003503 * amount*(buy_price+sell_price)
    SEBI_charges = 0.000001*turnover
    GST = 0.18 * (zerodha_brokerage + Exchange_txn_charge+ SEBI_charges)
    stamp_duty = 0.00003 * amount* (buy_price)
#     print(zerodha_brokerage + STT+ Exchange_txn_charge+ GST+ SEBI_charges)
    return((zerodha_brokerage + STT+ Exchange_txn_charge+ GST+ SEBI_charges+ stamp_duty))
...
def find_buy_strikes(kite, atm_strike, zerodha_instruments_list, buy_strike_threshold, strike_difference=50):
    ###### FIND BUY STRIKES (OPTIMIZED) ######
    start = time.time()
    
    # 1. PRE-FILTER INSTRUMENTS (Pure Logic, No API)
    # Ensure strikes are numeric for comparison
    # (Doing this on a copy to avoid SettingWithCopy warnings on main df if needed, 
    # but assuming main df is clean, we proceed)
    
    # Get 20 strikes ABOVE ATM (Likely Calls)
    upper_strikes = zerodha_instruments_list[
        zerodha_instruments_list["strike"] > atm_strike
    ].sort_values("strike").head(20)

    # Get 20 strikes BELOW ATM (Likely Puts)
    lower_strikes = zerodha_instruments_list[
        zerodha_instruments_list["strike"] < atm_strike
    ].sort_values("strike", ascending=False).head(20)

    # Combine symbols for batch fetching
    # We explicitly get CE for upper and PE for lower to minimize list size if preferred,
    # but your logic gets all symbols in that range. We'll fetch pairs to be safe.
    potential_instruments = pd.concat([upper_strikes, lower_strikes])
    
    # Create the list of trading symbols formatted for Kite (NFO:SYMBOL)
    symbols_to_fetch = ["NFO:" + s for s in potential_instruments["tradingsymbol"].unique()]

    # 2. FETCH LTP WITH RETRY LOGIC (20 Attempts)
    ltps = {}
    success = False
    
    for attempt in range(20):
        try:
            if not symbols_to_fetch: break # Safety check
            ltps = kite.ltp(symbols_to_fetch)
            success = True
            break
        except Exception as e:
            print(f"Buy Strike Fetch Attempt {attempt+1}/20 failed: {e}")
            time.sleep(1)

    # 3. SELECT STRIKES (OR FALLBACK)
    buy_entry_call = 0
    buy_entry_put = 0

    if success:
        try:
            print(f"Selecting best strikes closest to premium: {buy_strike_threshold}")
            
            # --- Find Best CALL ---
            # Filter for CE instruments only from our fetched list
            ce_candidates = potential_instruments[potential_instruments['instrument_type'] == 'CE']
            
            # Logic: Min( abs(LTP - Target) )
            # We use a generator expression for memory efficiency
            best_ce = min(
                ce_candidates.to_dict('records'),
                key=lambda x: abs(ltps.get("NFO:"+x["tradingsymbol"], {}).get('last_price', 99999) - buy_strike_threshold)
            )
            buy_entry_call = best_ce['strike']

            # --- Find Best PUT ---
            # Filter for PE instruments only from our fetched list
            pe_candidates = potential_instruments[potential_instruments['instrument_type'] == 'PE']
            
            best_pe = min(
                pe_candidates.to_dict('records'),
                key=lambda x: abs(ltps.get("NFO:"+x["tradingsymbol"], {}).get('last_price', 99999) - buy_strike_threshold)
            )
            buy_entry_put = best_pe['strike']
            
        except Exception as e:
            print(f"Error processing price data: {e}. Switching to fallback.")
            success = False # Trigger fallback below

    # 4. FALLBACK LOGIC
    if not success:
        print("CRITICAL: API Failed. Using Fallback (4 strikes away).")
        # Fallback: 4 strikes OTM (Assuming standard Buy hedge logic)
        # Call Buy = ATM + 4 strikes
        buy_entry_call = atm_strike + (4 * strike_difference)
        # Put Buy = ATM - 4 strikes
        buy_entry_put = atm_strike - (4 * strike_difference)

    print(f"Selected Buy Call: {buy_entry_call} | Selected Buy Put: {buy_entry_put}")
    print(f"Time taken: {time.time() - start:.2f}s")
    
    return buy_entry_call, buy_entry_put
...
def sell_fn(kite, zerodha_instruments_list, expiry):
    ...
    buy_strike_threshold = 15
    strike_balance_mode = 1
    
    ##### EXPIRY DAY CHANGES
    lots = math.floor(lot_reducer*lots_num)
    qty = lots*lot_size
    time.sleep(303 - time.localtime().tm_sec )

    profit_target = 45000 * lots
    loss_target = 4000 * lots    
    ...
    ###### FIND BUY STRIKES
    buy_entry_call, buy_entry_put = find_buy_strikes(
    kite=kite, 
    atm_strike=atm_strike, 
    zerodha_instruments_list=zerodha_instruments_list, 
    buy_strike_threshold=buy_strike_threshold,  # e.g., looking for 20 Rs premium
    strike_difference= strike_difference    # e.g., for Nifty
    )

    print('Buy Entry Call:', buy_entry_call, 'Buy Entry Put:', buy_entry_put)
    print('Sell Entry Call:', entry_call, 'Sell Entry Put:', entry_put)
    ...
    #####GET INITIAL BUY ENTRY PUT PRICE
    while True:
        sym_buy_put = next(item for item in zerodha_instruments_list[zerodha_instruments_list['strike'] == int(buy_entry_put)]['tradingsymbol'].values if 'PE' in item)
        for _ in range(10):
            try:
                a1 = kite.ltp('NFO:'+sym_buy_put)
                initial_buy_put_price = (a1['NFO:'+sym_buy_put]['last_price'])
                break
            except Exception as e:
                print(e)
                time.sleep(1)
    ...
    #####GET INITIAL BUY ENTRY CALL PRICE    
    while True:
        sym_buy_call = next(item for item in zerodha_instruments_list[zerodha_instruments_list['strike'] == int(buy_entry_call)]['tradingsymbol'].values if 'CE' in item)
        for _ in range(10):
            try:
                a1 = kite.ltp('NFO:'+sym_buy_call)
                initial_buy_call_price = (a1['NFO:'+sym_buy_call]['last_price'])
                break
            except Exception as e:
                time.sleep(1)
        break
    ...
    print('Buy Put Price:', initial_buy_put_price, 'Buy Call Price:', initial_buy_call_price)

    threshold_put_price = initial_entry_put_price- (entry_price_gap*0.01*initial_entry_call_price)
    threshold_call_price = initial_entry_call_price - (entry_price_gap*0.01*initial_entry_call_price)
    total_premium = threshold_put_price+threshold_call_price
    print('Threshold Entry Put Price:',threshold_put_price , 'Threshold Entry Call Price:',threshold_call_price, 'Total_Premium:', total_premium )

    stop_call_price = 1.5*threshold_call_price
    stop_put_price = 1.5*threshold_put_price
    print('Stop Call Price:', stop_call_price, 'Stop Put Price:', stop_put_price)

##    if total_premium > 250 and total_premium < 275: live_mode = 0
##    else: live_mode = 1
    ...
    while True:
        ...
        counter+=1
        now = datetime.now()
        time.sleep(64 - now.second)
        
        ####### GET CURRENT PRICES FOR ALL THE SYMBOLS
        
        for _ in range(10):
            try:
                a1 = kite.quote(['NFO:'+sym_put, 'NFO:'+sym_call, 'NFO:'+sym_buy_put, 'NFO:'+sym_buy_call])
                current_entry_put_price = (a1['NFO:'+sym_put]['last_price'])
                current_entry_call_price = (a1['NFO:'+sym_call]['last_price'])
                current_buy_put_price = (a1['NFO:'+sym_buy_put]['last_price'])
                current_buy_call_price = (a1['NFO:'+sym_buy_call]['last_price'])
                break
            except Exception as e:
                time.sleep(1)
        ...
        if current_entry_put_price < threshold_put_price and sell_put_flag == 0:
            ...
            for _ in range(10):
                try:
                    ...
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Put_Sold', entry_put_price, entry_hedge_put_price)
        ...
        if current_entry_call_price < threshold_call_price and sell_call_flag == 0:
            ...
            for _ in range(10):
                try:
                    ...
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Call_Sold', entry_call_price, entry_hedge_call_price)
        ...
        ###### CHECK BUY PUT ENTRY
        if current_buy_put_price >= (2* initial_buy_put_price) and buy_put_flag == 0:
            if live_mode == 1:
                ####PLACE BUY PUT ORDER
                place_order(kite, sym_buy_put, qty, 'BUY')
                
            #####GET BUY ENTRY PUT PRICE
            for _ in range(10):
                try:
                    a1 = kite.ltp('NFO:'+sym_buy_put)
                    buy_put_price = (a1['NFO:'+sym_buy_put]['last_price'])
                    break
                except Exception as e:
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Put_Bought', buy_put_price)
            intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_put, 'PE', initial_buy_put_price, buy_put_price, 0, 0 , final_profit, 'Open - Put_Bought']
            intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
            buy_put_flag = 1

        ###### CHECK BUY CALL ENTRY
        if current_buy_call_price >= (2* initial_buy_call_price) and buy_call_flag == 0:
            if live_mode == 1:
                ####PLACE BUY CALL ORDER
                place_order(kite, sym_buy_call, qty, 'BUY')
                
            #####GET BUY ENTRY CALL PRICE
            for _ in range(10):
                try:
                    a1 = kite.ltp('NFO:'+sym_buy_call)
                    buy_call_price = (a1['NFO:'+sym_buy_call]['last_price'])
                    break
                except Exception as e:
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Call_Bought', buy_call_price)
            intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_call, 'CE', initial_buy_call_price, buy_call_price, 0, 0 , final_profit, 'Open - Call_Bought']
            intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
            buy_call_flag = 1
        ...
        if buy_put_flag == 1: buy_put_profit = qty*(current_buy_put_price - buy_put_price) - commission(qty, buy_put_price, current_buy_put_price)
        if buy_call_flag == 1: buy_call_profit = qty*(current_buy_call_price - buy_call_price) - commission(qty, buy_call_price, current_buy_call_price)
        current_profit = sell_put_profit+sell_call_profit+buy_put_profit+buy_call_profit
        
        print(datetime.now(), 'Current_profit:', round(current_profit,2), 'PE:', current_entry_put_price, 'CE:',current_entry_call_price, 'BPE:',current_buy_put_price, 'BCE:',current_buy_call_price,sell_put_profit,sell_call_profit,buy_put_profit,buy_call_profit)
...
##            final_profit = final_profit + sell_put_profit
            ...
            print(datetime.now(), 'Sell_Put_Stopped', sell_put_profit, final_profit, exit_sell_put_price, stop_put_price)
            sell_put_flag = 2
            profit_target = 0
            continue
...
##            final_profit = final_profit + sell_call_profit
            ...
            print(datetime.now(), 'Sell_Call_Stopped',sell_call_profit, final_profit, exit_sell_call_price, stop_call_price)
            sell_call_flag = 2
            profit_target = 0
            continue

        if buy_put_flag == 1 and current_buy_put_price >= 280 :
            if live_mode == 1:
                ####PLACE SELL PUT ORDER
                place_order(kite, sym_buy_put, qty, 'SELL')
                
            final_profit = final_profit + buy_put_profit
            #####GET BUY EXIT PUT PRICE
            for _ in range(10):
                try:
                    a1 = kite.ltp('NFO:'+sym_buy_put)
                    exit_buy_put_price = (a1['NFO:'+sym_buy_put]['last_price'])
                    break
                except Exception as e:
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Buy_Put_closed', final_profit, exit_buy_put_price)
            intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_put, 'PE', initial_buy_put_price, buy_put_price, current_buy_put_price, buy_put_profit , final_profit, 'Close - Buy_Put_Stopped']
            intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
            buy_put_flag = 2
            
        if buy_call_flag == 1 and current_buy_call_price >= 280 :
            if live_mode == 1:
                ####PLACE SELL CALL ORDER
                place_order(kite, sym_buy_call, qty, 'SELL')
                
            final_profit = final_profit + buy_call_profit
            #####GET BUY EXIT CALL PRICE
            for _ in range(10):
                try:
                    a1 = kite.ltp('NFO:'+sym_buy_call)
                    exit_buy_call_price = (a1['NFO:'+sym_buy_call]['last_price'])
                    break
                except Exception as e:
                    print(e)
                    time.sleep(1)
            print(datetime.now(), 'Buy_Call_closed', final_profit, exit_buy_call_price)
            intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_call, 'CE', initial_buy_call_price, buy_call_price, current_buy_call_price, buy_call_profit , final_profit, 'Close - Buy_Call_Stopped']
            intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
            buy_call_flag = 2
            
            
        ######### EXIT IF OVERALL PROFIT OR LOSS HIT OR AT THE END OF DAY
        if (abs(current_profit) > 0 and (current_profit > profit_target or current_profit < -loss_target)) or (datetime.now().hour == 15 and datetime.now().minute >= 19):
            ...
            if buy_put_flag == 1:
                ...
                for _ in range(10):
                    try:
                        ...
                        print(e)
                        time.sleep(1)
                print(datetime.now(), 'Buy_Put_closed', buy_put_profit, exit_buy_put_price)
                intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_put, 'PE', initial_buy_put_price, buy_put_price, current_buy_put_price, buy_put_profit , final_profit, 'Close - Buy_Put_Closed']
                intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
            ...
            if buy_call_flag == 1:
                ...
                for _ in range(10):
                    try:
                        ...
                        print(e)
                        time.sleep(1)
                print(datetime.now(), 'Buy_Call_closed', buy_call_profit, exit_buy_call_price)
                intraday_positions.loc[intraday_positions.shape[0]] = [str(datetime.now(pytz.timezone('Asia/Kolkata'))).split('.')[0], buy_entry_call, 'CE', initial_buy_call_price, buy_call_price, current_buy_call_price, buy_call_profit , final_profit, 'Close - Buy_Call_Closed']
                intraday_positions.to_csv(gd_path+'nifty_buy_sell_results/Intraday_options_tradebook.csv', index=False)
...
while True:
    ...
    if current_day != '0' and current_day != '6':
        ...
        if '09:15' in str(now):
            ...
            zerodha_instruments_list = pd.read_csv(gd_path+'instrument_tokens.csv')
            ...
            print(expiry)
            ##### IF TODAY IS EXPIRY CALL FUNCTION
            if datetime.now().date() == pd.to_datetime(expiry).date(): sell_fn(kite, zerodha_instruments_list,expiry)
