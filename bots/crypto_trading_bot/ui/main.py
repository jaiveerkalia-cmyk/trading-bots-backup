"""
NiceGUI entrypoint — single-page trading dashboard.
"""
...
from ui.components import pnl_chart, price_chart, activity_log
from ui.components import tv_widget
...
async def index() -> None:
    ...
    # Lightweight Charts
    ui.add_head_html(
        '<script src="https://unpkg.com/lightweight-charts@4.1.3/dist/'
        'lightweight-charts.standalone.production.js"></script>'
    )
    ...
    with ui.element('div').classes('w-full px-3 py-2'):
        ...
        # ── Row 2: Charts (tabbed) | PnL chart ───────────────────────────────
        with ui.row().classes('w-full gap-3 mb-3 items-start'):

            # Tabbed chart panel — Live feed | TradingView reference
            with ui.element('div').classes('flex-1 min-w-0'):
                with ui.card().classes('w-full bg-gray-900 p-0 rounded-lg overflow-hidden'):
                    with ui.tabs().classes('bg-gray-800 text-gray-400') as tabs:
                        tab_live = ui.tab('live',         label='Live Chart')
                        tab_tv   = ui.tab('tradingview',  label='TradingView')

                    with ui.tab_panels(tabs, value='live').classes('w-full bg-gray-900'):
                        with ui.tab_panel('live').classes('p-0'):
                            pc = price_chart.build(_state, shared)

                        with ui.tab_panel('tradingview').classes('p-0'):
                            tv = tv_widget.build(_state, shared)

            # PnL curve — fixed width alongside charts
