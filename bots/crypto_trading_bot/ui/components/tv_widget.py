...
# Perpetual futures symbols (PERP suffix on TradingView)
_FUTURES = {
    'binance_futures:BTC/USDT':  'BINANCE:BTCUSDTPERP',
    'binance_futures:ETH/USDT':  'BINANCE:ETHUSDTPERP',
    'binance_futures:SOL/USDT':  'BINANCE:SOLUSDTPERP',
    'binance_futures:BNB/USDT':  'BINANCE:BNBUSDTPERP',
    'binance_futures:XRP/USDT':  'BINANCE:XRPUSDTPERP',
    'binance_futures:DOGE/USDT': 'BINANCE:DOGEUSDTPERP',
    'binance_futures:BTC/USD':   'BITMEX:XBTUSD',
    'delta:BTC/USD':             'BITMEX:XBTUSD',
    'delta:ETH/USD':             'BITMEX:ETHUSD',
    'delta:BTC/USDT':            'BINANCE:BTCUSDTPERP',
    'delta:ETH/USDT':            'BINANCE:ETHUSDTPERP',
}
_TV_SYMBOL_MAP = {**_SPOT, **_FUTURES}
...
def _tv_symbol(exchange: str, symbol: str) -> str:
    ...
    # Auto-suffix PERP for futures exchanges
    if 'futures' in exchange:
        return f"BINANCE:{native}PERP"
...
def build(state: 'UIState', shared: dict) -> dict:
    ...
    with ui.card().classes('w-full bg-gray-900 p-0 rounded-lg overflow-hidden'):
        with ui.row().classes('w-full items-center px-3 py-2 bg-gray-800 gap-3'):
            ui.label('TradingView Chart').classes('text-gray-300 text-sm font-medium')
            ui.label('(reference — TradingView data)').classes('text-gray-600 text-xs')
            ui.select(
                options=['1m', '5m', '15m', '1h', '4h', '1d'],
                value='1m', label='Interval',
                on_change=lambda e: (
        ...
        with ui.element('div').style('width:100%; height:420px; position:relative;'):
