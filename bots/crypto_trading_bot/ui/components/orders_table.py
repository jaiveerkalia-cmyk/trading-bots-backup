...
_COLS = [
    ...
    {'name': 'filled',    'label': 'Filled',   'field': 'filled_qty', 'align': 'right'},
    {'name': 'status',    'label': 'Status',   'field': 'status',     'align': 'center'},
    {'name': 'mode',      'label': 'Mode',     'field': 'is_paper',   'align': 'center'},
    {'name': 'cancel',    'label': '',         'field': 'id',         'align': 'center'},
]
...
def build(state: 'UIState', redis: aioredis.Redis) -> dict:
    with ui.card().classes('w-full bg-gray-900 p-3 rounded-lg'):
        ...
        table.add_slot('body-cell-status', """
            <q-td :props="props">
              <q-badge :color="props.value==='working'?'blue':'grey'" :label="props.value"/>
            </q-td>
        """)
...
async def _cancel(row: dict, redis: aioredis.Redis) -> None:
    await commands.cancel_order(redis, row.get('slot_id', ''), row.get('id', ''))
    ui.notify(f"Cancel sent for {row.get('symbol','')} order", type='warning')
