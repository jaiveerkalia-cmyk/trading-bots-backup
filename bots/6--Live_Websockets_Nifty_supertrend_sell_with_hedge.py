...
# ─────────────────────────────────────────────────────────────────────────────
DAY_CONFIG = {
    0: {'lots_num': 10, 'live_mode': 0, 'start_time': '09:20', 'supertrend_period': 4, 'stop_percent': 0.7, 'reverse_threshold_percentage': 0.1, 'global_stop_per_lot': 40000, 'hedgeless_mode': 0, 'risk_mode_on': 1, 'max_risk_amount': 25000},  # Monday
    1: {'lots_num': 10, 'live_mode': 0, 'start_time': '09:20', 'supertrend_period': 4, 'stop_percent': 0.7, 'reverse_threshold_percentage': 0.3, 'global_stop_per_lot': 40000, 'hedgeless_mode': 1, 'risk_mode_on': 1, 'max_risk_amount': 10000},  # Tuesday
    2: {'lots_num': 10, 'live_mode': 0, 'start_time': '09:20', 'supertrend_period': 4, 'stop_percent': 0.7, 'reverse_threshold_percentage': 0.1, 'global_stop_per_lot': 40000, 'hedgeless_mode': 0, 'risk_mode_on': 1, 'max_risk_amount': 25000},  # Wednesday
    3: {'lots_num': 10, 'live_mode': 0, 'start_time': '09:20', 'supertrend_period': 4, 'stop_percent': 0.7, 'reverse_threshold_percentage': 0.1, 'global_stop_per_lot': 40000, 'hedgeless_mode': 0, 'risk_mode_on': 1, 'max_risk_amount': 25000},  # Thursday
    4: {'lots_num': 10, 'live_mode': 0, 'start_time': '09:20', 'supertrend_period': 4, 'stop_percent': 0.7, 'reverse_threshold_percentage': 0.1, 'global_stop_per_lot': 40000, 'hedgeless_mode': 0, 'risk_mode_on': 1, 'max_risk_amount': 20000},  # Friday
}
...
def print_tomorrow_mode():
    ...
    print(f"  risk_mode_on      : {next_cfg['risk_mode_on']}")
    print(f"  max_risk_amount   : {next_cfg['max_risk_amount']}")
    print(f"{'='*60}\n")
...
def sell_fn(kite, zerodha_instruments_list, expiry, api_key, access_token):
    ...
    print(f"  Mode={mode_str} | lots_num={lots_num} | start_time={start_time} | "
          ...
          f"risk_mode_on={risk_mode_on} | max_risk_amount={max_risk_amount}")
    print(f"{'='*60}\n")
    # ─────────────────────────────────────────────────────────────────────────
    ...
    print('Qty (pre-entry, from lots_num):', qty)
...
##    df_ohlc['atr'] =  talib.ATR(df_ohlc.high, df_ohlc.low, df_ohlc.close, timeperiod= 10)
    ...
    for pos_num in range(0, 3):
        ...
        #####GET INITIAL ENTRY PUT PRICE
            ...
            #####GET ENTRY PUT PRICE FIRST (needed for risk_mode qty calc)
            for _ in range(10):
                try:
            ...
            ##### CALCULATE QTY BASED ON RISK MODE
            if risk_mode_on == 1:
                stop_sell_put_price = (1 + stop_percent) * entry_put_price
                risk_qty = math.floor((max_risk_amount / (stop_sell_put_price - entry_put_price)) / lot_size) * lot_size
                qty = max(risk_qty, lot_size)
                print(f'Risk Mode ON: entry_put_price={entry_put_price:.2f}, stop={stop_sell_put_price:.2f}, calculated qty={qty}')
        ...
        #####GET INITIAL ENTRY CALL PRICE
            ...
            #####GET ENTRY CALL PRICE FIRST (needed for risk_mode qty calc)
            for _ in range(10):
                try:
            ...
            ##### CALCULATE QTY BASED ON RISK MODE
            if risk_mode_on == 1:
                stop_sell_call_price = (1 + stop_percent) * entry_call_price
                risk_qty = math.floor((max_risk_amount / (stop_sell_call_price - entry_call_price)) / lot_size) * lot_size
                qty = max(risk_qty, lot_size)
                print(f'Risk Mode ON: entry_call_price={entry_call_price:.2f}, stop={stop_sell_call_price:.2f}, calculated qty={qty}')
